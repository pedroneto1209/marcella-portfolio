...
function WorkCard({ name, title1, title2, description, link1, link2 }: Props) {
  return (
    <motion.div
      aria-current="page"
      whileHover="hover"
    ...
    >
      <motion.div className="relative overflow-hidden rounded-3xl md:h-[780px] h-[500px]">
        ...
        {link1 && (
          <Link href={link1}>
            <motion.div
              className={`absolute ${link2 ? "bottom-[230px]" : "bottom-[150px]"
                } right-8`}
              variants={{
                rest: { opacity: 0 },
                hover: {
            ...
            >
              <Button label="Let&apos;s see the process" />
            </motion.div>
          </Link>
        )}
        ...
        {link2 && (
          <Link href={link2}>
            ...
            >
              <Button label="Let&apos;s see the result" />
            </motion.div>
          </Link>
        )}
      </motion.div>
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-3 left-4 w-full h-15 flex space-x-10 items-center">
        ...
        <a className="font-main font-regular text-[25px] text-white">{label}</a>
      </div>
      <motion.div
